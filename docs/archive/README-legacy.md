# ESP32-C3 + Air101-LCD 开发项目（归档）

> **本文件为历史完整版 README**，自 v1.0.1 起已拆分到根目录 `README.md` 与 `docs/`。仅供查阅 PCB、原理图、环境附录等长章节，日常开发请以根 README 与 `docs/README.md` 为准。
...
| 摇杆 | `joystick/joystick.c` | `joystick_init`, `joystick_poll`（测试见下） |

**摇杆测试（可选）**：menuconfig → **Joystick** → 开启 `Boot into joystick test`，或 `sdkconfig` 中 `CONFIG_JOYSTICK_ENABLE_TEST=y` 后编译。默认关闭，上电为 WiFi/OTA/设备信息界面。实机 GPIO：UP=13、DOWN=8、RIGHT=5、LEFT=9、CENTER=4。
